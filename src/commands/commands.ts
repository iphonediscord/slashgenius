...
const registerCommands = async () => {
  ...
  let createdCommands: ApplicationCommand[] = await api.bulkOverwriteCommands(
    commands
  );
...
const createTag = async (interaction: Interaction) => {
  ...
  dbCreate: try {
    ...
    if (!isValidTagName(tagName)) {
      response =
        "Tag names must start with a lowercase letter and only contain up to 32 alphanumeric characters.";
      break dbCreate;
    }
  ...
  new ResponseAPI(interaction.token).editOriginalResponse({
    content: response,
  });
};
...
const updateCommands = async (interaction: Interaction) => {
  ...
  new ResponseAPI(interaction.token).editOriginalResponse({
    content: "Successfully updated all commands.",
  });
};
...
const displayTag = async (interaction: Interaction) => {
  ...
  let response =
    content ||
    `There was an error displaying the \`${name}\` tag. Please contact server staff.`;
